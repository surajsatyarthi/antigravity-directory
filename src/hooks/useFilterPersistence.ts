...
export function useFilterPersistence() {
  ...
  const hasUrlParams = categoriesParam !== '' || tagsParam !== '' || badgesParam !== '' || qParam !== '' || sortParam !== FILTERS.DEFAULT_SORT || groupParam !== '';
  ...
  useEffect(() => {
    ...
    const hasValues = filters.categories.length > 0 || filters.tags.length > 0 || filters.badgeTypes.length > 0 || filters.search !== '' || filters.sort !== FILTERS.DEFAULT_SORT || filters.group !== '';
  ...
  }, [categoriesParam, tagsParam, qParam, sortParam, isReady, searchParams.size, badgesParam, groupParam]);
