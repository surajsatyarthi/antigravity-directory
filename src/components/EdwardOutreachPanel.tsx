    ...
    <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden">
      ...
      <div className="p-8 border-b border-white/[0.03] bg-gradient-to-r from-blue-600/5 to-transparent flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">Edward <span className="text-blue-500">Outreach Intelligence</span></h3>
            <p className="text-[10px] text-slate-500 font-mono uppercase tracking-[0.2em] mt-1">Status: Scanning for High-Signal Leads</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={enriching}
            className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center gap-2"
          >
            {enriching ? (
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-100 border border-slate-200 text-[9px] font-black text-blue-400 uppercase tracking-widest">
          ...
          <div className="mb-8 p-6 rounded-2xl bg-purple-500/5 border border-purple-500/20">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="text-center">
                <p className="text-2xl font-black text-slate-900">{enrichmentResults.enriched}</p>
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-mono">Emails Found</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-black text-gray-600">{enrichmentResults.failed}</p>
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-mono">Not Found</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-black text-purple-400">{enrichmentResults.creditsUsed}</p>
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-mono">Credits Used</p>
              </div>
            </div>
            ...
            {prospects.map((target) => (
              <div 
                ...
                className={`p-6 rounded-2xl bg-white/[0.02] border transition-all group relative overflow-hidden ${
                  successId === target.id ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-slate-200 hover:border-blue-500/30'
                }`}
              >
                {successId === target.id && (
                  <div className="absolute inset-0 bg-emerald-500/10 flex items-center justify-center z-10 backdrop-blur-sm animate-in fade-in zoom-in duration-300">
                    <div className="text-center">
                      ...
                      <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Pitch Delivered</p>
                    </div>
                  </div>
                )}

                <div className="flex items-center justify-between mb-4">
                  <h4 className="font-black text-slate-900 tracking-tight uppercase italic">{target.name}</h4>
                ...
                <p className="text-[10px] text-slate-500 font-mono uppercase tracking-widest leading-relaxed mb-8 flex items-start gap-2">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="flex-1 py-3 bg-slate-100 hover:bg-slate-100 text-slate-900 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all border border-slate-200 flex items-center justify-center gap-2"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-slate-900 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-500/10 active:scale-95 flex items-center justify-center gap-2"
                  >
      ...
      {viewingPitch && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-24 bg-white/90 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white border border-slate-200 w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl shadow-blue-500/10 flex flex-col">
            <div className="p-8 border-b border-slate-200 bg-white/[0.02] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-blue-500" />
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter italic">Edward's <span className="text-blue-500">Draft</span></h3>
              </div>
              <button 
            ...
            <div className="p-8 font-mono text-xs text-slate-400 bg-white/50 overflow-y-auto max-h-[60vh] leading-relaxed">
