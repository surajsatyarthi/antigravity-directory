...
export function ToolsShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-black text-gray-300">
      ...
      <aside 
        className={cn(
          "flex-shrink-0 border-r border-white/[0.06] bg-white/[0.03] transition-all duration-300 ease-in-out overflow-y-auto custom-scrollbar",
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-transparent overflow-hidden">
        ...
        <header className="h-14 flex items-center px-4 border-b border-white/[0.06] justify-between">
            <button 
                        ...
                        {allTools.filter(t => t.href !== pathname).slice(0, 3).map(tool => {
                            ...
                            return (
                                <Link 
                                    key={tool.href}
                                    href={tool.href}
                                    className="p-4 rounded-none border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.06] hover:border-white/[0.12] transition group"
                                >
