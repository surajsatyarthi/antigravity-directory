  ...
  return (
    <>
      <button
        ...
        className="w-full py-3 px-4 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2 shadow-sm"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div role="dialog" aria-modal="true" className="bg-white border border-slate-200 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl relative">
            <button 
            ...
            <div className="p-6">
              <h2 className="text-xl font-bold text-slate-900 mb-1">Purchase Resource</h2>
              <p className="text-sm text-slate-500 mb-6">{resourceName}</p>
                ...
                {countryCode === 'IN' && (
                  <>
                    <button
                      ...
                      className="w-full py-3 bg-slate-900 text-white font-bold rounded-lg hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                {/* International: PayPal Primary, Razorpay Secondary */}
                 ...
                 {countryCode !== 'IN' && (
                    <button
                      onClick={handleRazorpayPayment}
                      disabled={!isRazorpayReady || loading}
                      className="w-full py-3 bg-slate-50 border border-slate-200 text-slate-700 font-bold rounded-lg hover:bg-slate-100 transition-colors flex items-center justify-center gap-2 mt-3 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
