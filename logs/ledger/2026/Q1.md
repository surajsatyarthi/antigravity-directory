...
| TASK-083 | Copy button on listing cards — copy without clicking through | ✅ DONE | Commits bb08873. 3 files changed: queries.ts (content added to SELECT), CopyButton.tsx (iconOnly prop + stopPropagation), ResourceCard.tsx (conditional copy button at z-30). S1: copy icon visible at 200% zoom. S2: URL bar confirms /rules after click (page-stays confirmed; green ✓ animation not captured due to timing). S3: sponsored MCP card shows only arrow, no copy button. |
...
| TASK-098 | /learn/ comparison pages — antigravity vs competitors | ⏳ PENDING (Phase 2 — after PSEO 2.0 core pages) | Reviewer v2.2 confirmed: high intent during quota crisis. Move to Phase 2 (after TASK-122 500-page rebuild complete), not Phase 3. Thin comparison pages on top of thin resource pages = double de-indexing risk. Gate: TASK-122 must be DONE before TASK-098 starts. |
...
| TASK-114 | Add tags to every resource | ⏳ PENDING (Phase 2) | Automated tag extraction from title+description+content. Unblocks TASK-051 filters. Both competitors have clickable tags. tags + resourceTags tables exist in schema but unpopulated. |
| TASK-115 | Visual audit — screenshot all key pages fresh | ⏳ PENDING (HIGH) | User flagged unknown UI state across 80+ past tasks — screenshot verification was not rigorous. Fresh screenshots of: homepage, /rules, /mcp-servers, /t/[slug] detail, /advertise, /about, mobile view. PM reads each and describes what is visible before any new UI work proceeds. |
| TASK-116 | Mechanical gates — consequence check + 3-screenshot standard | ⏳ PENDING (HIGH) | Two text-based rules that failed: (1) proactive consequence thinking — add mandatory CONSEQUENCE CHECK section to CURRENT_TASK.md template, enforced by evidence-gate.sh; (2) 3-screenshot standard — evidence-gate.sh blocks DONE if fewer than 3 screenshot files exist when SCREENSHOTS: REQUIRED. Converts will-based rules to mechanical gates. |
| TASK-117 | Fix canonical tags — remove wrong layout canonical, add to 11 pages | 🔄 IN PROGRESS | Spec written 2026-03-31. 12 file changes: layout.tsx (remove canonical: '/'), +alternates to advertise, google-antigravity, tools, tools/*, privacy (remove 'use client' + add metadata), terms (same), submit (add metadata). Verified: privacy + terms have no client hooks — safe to convert. |
| TASK-118 | Fix 80 noindex pages — identify + remediate | ⏳ PENDING (CRITICAL SEO) | GSC reports 80 pages excluded by noindex tag. No explicit noindex found in code. Likely cause: Next.js auto-adds X-Robots-Tag: noindex to notFound() responses — deleted resource slugs Google previously crawled now return 404+noindex. Fix: (1) get example URLs from GSC "Excluded by noindex" drill-down, (2) confirm cause, (3) implement 410 Gone handler for deleted slugs OR ensure deleted pages return proper 410 not 404+noindex. |
| TASK-119 | Fix 4 redirects + 1 soft 404 in GSC coverage report | ⏳ PENDING (MEDIUM SEO) | GSC reports: 4 "Page with redirect" + 1 "Soft 404". Need example URLs from GSC to identify. Likely: /c/mcp-servers redirect (TASK-075) + www/non-www residuals. Soft 404: a page returning 200 but appearing empty to Google. Investigate and fix each. |
| TASK-120 | Creator backlink engine — GitHub Issue outreach | ⏳ PENDING (HIGH) | Renamed from TASK-062. Script: parse GitHub owner/repo from resource URLs → open GitHub Issue on each repo with badge embed code. 500 issues → expected 75-125 README backlinks. GitHub backlinks = high domain authority signal. This is the primary lever to move the 3,003 discovered-not-indexed pages into Google's index. |
| TASK-121 | PSEO 2.0 — taxonomy + JSON schemas per category | ✅ DONE | Generated by Grok 4.20 using real-time X search (since:2026-03-01). File: docs/seo/PSEO_2_TAXONOMY_SCHEMAS.json. 10 personas, 10 pain points (ghost consumption, 7-day lockout, error burn, Opus instant lock), 8 workflows, 5 integrations, 10 category schemas all with required tokenImpact field. Vocabulary grounded in actual developer X posts March 2026. Commit: pending. |
| TASK-122 | PSEO 2.0 — content enrichment pipeline (top 500 resources) | ⏳ PENDING (CRITICAL) | After TASK-121: script sends top 500 resources (MCP Servers + Rules by view count) through Gemini Flash with strict JSON schema → validates output → stores enriched JSON in content field. Resources without valid structured content get flagged. Target: every MCP page has description ≥120 chars, 3+ use cases, difficulty, exampleCommand, tokenImpact, 3+ tags. This directly addresses 3,003 discovered-not-indexed pages. |
| TASK-123 | PSEO 2.0 — React renderers for detail pages | ⏳ PENDING (HIGH) | After TASK-122: build specialized display components for /t/[slug] that render enriched JSON content. Priority components: (1) InteractiveMCPInstaller — shows npx/uvx command with one-click copy, (2) UseCaseChecklist — renders useCases as checkboxes, (3) TokenImpactBadge — shows token cost (high/med/low) — critical during quota crisis, (4) TagCloud — taxonomy-driven related resources. Pages must feel like tools not lists. Renderer code will be generated by Grok (external reviewer) — import directly. |
| TASK-125 | Rebuild 3 critical Claude Code skills — programmatic-seo, seo-audit, schema-markup | ⏳ PENDING (PM task — no Antigravity) | All 33 project-level skills were broken symlinks to .agents/skills/ (deleted TASK-109). Symlinks removed 2026-03-31. Rebuild 3 that directly serve PSEO 2.0 work: (1) .claude/skills/programmatic-seo — Jake Ward PSEO 2.0 principles, tokenImpact field rationale, batch enrichment rules; (2) .claude/skills/seo-audit — GSC error types (noindex/canonical/discovered-not-indexed), fix checklist, Next.js metadata rules; (3) .claude/skills/schema-markup — JSON-LD patterns per category, SoftwareApplication schema, BreadcrumbList. PM writes all 3 as markdown files directly. No Antigravity needed. |
| TASK-124 | Fix "Sponsored" label → "Featured" on resource cards | ⏳ PENDING (Phase 0 — 4 hours) | Change one word in src/components/ResourceCard.tsx line ~155: "Sponsored" → "Featured". Removes false-payment implication (external reviewer v2.2 confirmed legal risk of "SPONSORED" label on non-paying company logos). Logos stay — only the label changes. Simplest possible fix. |
| TASK-076 | Blog — quota-survival content pivot | ⏳ PENDING (CRITICAL — REWRITE) | Updated 2026-03-31 based on external audit. PIVOT: drop generic "best MCPs 2026" angle. New articles target what developers are actually searching during the quota crisis: (1) "Antigravity quota lockout: what is happening + when will it be fixed" — ZERO competition, maximum current intent; (2) "Low-token MCP servers for Google Antigravity (use less quota)" — captures quota-conscious developer search; (3) "Antigravity vs Cursor in 2026: which IDE to use while quota is broken" — captures switching traffic; (4) "How to survive the Google Antigravity 7-day token reset" — direct pain point, high CTR; (5) "How to add MCP servers in Google Antigravity IDE" — evergreen setup guide, zero competition. PM writes all 5 directly in Claude Code session. No Antigravity needed. |
