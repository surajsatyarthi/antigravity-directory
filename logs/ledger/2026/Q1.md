...
| TASK-121 | PSEO 2.0 — taxonomy + JSON schemas per category | ⏳ PENDING (CRITICAL) | External review (2026-03-31) confirmed current PSEO is 2024 thin-directory spam. Phase 1 foundation: PM defines (a) deep Antigravity taxonomy — 80-120 entries covering: quota pain, enterprise/hobbyist personas, agent orchestration, token optimization, multi-agent handoffs, browser automation, framework integrations; (b) strict JSON schema for each of 10 categories including `tokenImpact` field (critical during quota crisis). Output: docs/seo/PSEO_2_TAXONOMY.md + docs/seo/PSEO_2_SCHEMAS.md. PM writes both documents directly — no Antigravity needed for this step. |
