...
| TASK-117 | Fix canonical tags — remove wrong layout canonical, add to 11 pages | ⏳ PENDING (CRITICAL SEO) | layout.tsx has `canonical: '/'` which poisons 11 pages with no own canonical (/advertise, /submit, /privacy, /terms, /google-antigravity, /tools + 5 tool pages). All 11 tell Google their canonical is the homepage — causing 23 "Duplicate without user-selected canonical" in GSC. Fix: remove `canonical: '/'` from layout.tsx alternates. Add correct absolute canonical to each of the 11 pages. 12 file changes. |
