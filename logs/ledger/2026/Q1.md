...
| TASK-097 | Competitor resource gap ingestion | ⏳ PENDING | Competitor 1 (antigravityai.directory) has 1,085 MCP slugs + 730 prompt slugs in their sitemap (full slug list in docs/strategy/COMPETITOR_RESEARCH_2026.md). Cross-reference against our DB. For every slug we're missing: fetch original resource from GitHub/source, ingest into our DB with status=LIVE. Do NOT copy their descriptions — fetch from original source only. This grows our collection with resources users actively search for. |
