...
| TASK-083 | Copy button on listing cards — copy without clicking through | ⏳ PENDING (blocked on TASK-082 + more category runs) | antigravityai.directory has copy buttons directly on category/listing cards. Users copy the content without visiting the detail page. Requires content backfill to cover more categories first. Only show the copy button on cards where `content` is populated. |
...
| TASK-096 | Run content backfill on remaining 6 categories | ⏳ PENDING | TASK-082 committed the backfill script but only ran it on MCP Servers + Rules. 6 categories still have empty `content` field — no CopyButton renders: Prompts, Skills, Workflows, Agents, Boilerplates, Troubleshooting. Confirmed by quality audit screenshots (2026-03-16). Run `scripts/backfill-content.ts` against each remaining category. CopyButton will then render on detail pages for those resources. |
| TASK-097 | Competitor resource gap ingestion | ⏳ PENDING | Competitor 1 (antigravityai.directory) has 1,085 MCP slugs + 730 prompt slugs in their sitemap (full slug list in docs/02-strategy/COMPETITOR_RESEARCH_2026.md). Cross-reference against our DB. For every slug we're missing: fetch original resource from GitHub/source, ingest into our DB with status=LIVE. Do NOT copy their descriptions — fetch from original source only. This grows our collection with resources users actively search for. |
...
| TASK-102 | Fix category page resource count — header shows organic-only, must show total LIVE | ⏳ PENDING | Category page header displays "X resources" but the count is organic-only (featured=false). Featured resources are LIVE in DB but excluded from the count. A user on Boilerplates sees "36 resources" when there are 64 LIVE. This understates the directory to every user and every sponsor. Fix: the count query must return total LIVE resources in category regardless of featured flag. |
| TASK-103 | Manual indexing requests — homepage, categories, top resources | ⏳ PENDING | Sitemap tells Google URLs exist. Manual URL Inspection requests tell Google to crawl NOW. New site crawl budget is limited — Google will queue most pages. Fix: use GSC URL Inspection to manually request indexing for the 30 highest-priority pages: homepage + 10 category pages + top 20 MCP Server resources. Bing Webmaster Tools URL Inspection for same pages. GSC limit ~10/day — spread over 3 days. Founder action only (requires GSC/Bing login). |
...
| TASK-110 | Session protocol confirmation gate hook | ⏳ QUEUED | No mechanical gate prevents PM from skipping protocol document reads at session start. Root cause of TASK-106 session shortcut. New `session-protocol-gate.sh`: fires on PreToolUse Write/Edit, blocks until PM has written `SESSION-PROTOCOL-CONFIRMED: [date]` in CURRENT_TASK.md this session. Registered in `.claude/settings.json`. Depends on TASK-108. |
