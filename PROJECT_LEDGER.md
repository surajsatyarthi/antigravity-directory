...
| TASK-081 | Advertise page cleanup — remove 2 unbuilt ad slots + delete false stats claims | ⏳ PENDING | Remove Featured Listing + Promoted Card (neither is fully built/deliverable). Delete GSC footnote (line 153). Fix footer note (line 251). |
| TASK-082 | Content backfill — populate `content` field for all 3,116 resources | ⚠️ SCRIPT DONE, GIT COMMIT MISSING | Script `scripts/backfill-content.ts` written + verified. 60 MCP resources updated in real run. Build/lint exit 0. Screenshots: MCP + Rule detail pages showing content + CopyButton ✅. HTTP 307 (pre-existing www/non-www redirect — TASK-084). ❌ No git commit — script never committed to repo. Antigravity must commit `scripts/backfill-content.ts` before this is DONE. |
...
| TASK-084 | Fix www/non-www URL mismatch — health score 0 | ⏳ PENDING (URGENT) | Ahrefs audit shows health score 0. Root cause: all hardcoded URLs use `https://googleantigravity.directory` (no www) but live site serves on `https://www.googleantigravity.directory` (with www). 25 occurrences across 12 files. Fix: global find-and-replace. Also fix title too long on 3,157 pages. |
